...
function mutable_source(initial_value) {
  ...
  if (
    ...
    current_component_context.l !== null
  ) {
...
function destroy_effect(effect2) {
  ...
  effect2.next =
    ...
    effect2.dom =
    effect2.deps =
    effect2.parent = // @ts-expect-error
    effect2.fn =
      null;
}
...
function destroy_derived(signal) {
  ...
  signal.first =
    signal.last =
    signal.deps =
    signal.reactions = // @ts-expect-error `signal.fn` cannot be `null` while the signal is alive
    signal.fn =
      null;
}
...
function is_runes() {
  return (
    current_component_context !== null && current_component_context.l === null
  );
}
...
function mark_reactions(signal, to_status, force_schedule) {
  ...
  for (var i = 0; i < length; i++) {
    ...
    if (
      (flags & DIRTY) !== 0 ||
      ((!force_schedule || !runes) && reaction === current_effect)
    ) {
      continue;
    }
  ...
  get(target, prop, receiver) {
    ...
    if (
      ...
      s = (metadata.i ? source : mutable_source)(
        proxy(target[prop], metadata.i, metadata)
      );
...
function handle_event_propagation(handler_element, event) {
  ...
  function next(next_target) {
    ...
    var parent_element =
      next_target.parentNode || /** @type {any} */ next_target.host || null;
    try {
      var delegated = next_target['__' + event_name];
      if (delegated !== void 0 && !(/** @type {any} */ next_target.disabled)) {
...
function _mount(
  Component,
  { target, anchor, props = {}, events, context, intro = false }
) {
  init_operations();
